...
- W6-3 carries formal `Depends: W2-2, W3-4` (config/auth-401 decisions) and is M3-scheduled; it additionally mirrors W3-5's `.env` decision, covered by the same M3 gate since W3-5 is an M3 code item. Block on those decisions/landings before finalizing the precedence/`.env`/auth-401 prose.
...
- [ ] observability.md log-correlation recipe matches the W5-12 landed filter behavior and uses no 3.10+-only API.
